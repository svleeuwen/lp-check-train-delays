...
class NSApi(object):
    ...
    BASE_URL = "http://webservices.ns.nl/ns-api-treinplanner?fromStation=%(from_station)s&toStation=%(to_station)s&dateTime=%(date_time)s&hslAllowed=%(include_highspeed)s&previousAdvices=%(num_previous_advices)s"
    ...
    def parse_result(self, xml):
        ...
        for option in travel_options:
            if not option.find('Status').text == self.DELAYED_MESSAGE:
                ...
                continue
            delays.append({
                'departure_planned': self._parse_time(option.find('GeplandeVertrekTijd').text),
                'departure_planned_str': self._parse_time(option.find('GeplandeVertrekTijd').text).strftime('%H:%M'),
                'departure_actual': self._parse_time(option.find('ActueleVertrekTijd').text),
                'departure_actual_str': self._parse_time(option.find('ActueleVertrekTijd').text).strftime('%H:%M'),
